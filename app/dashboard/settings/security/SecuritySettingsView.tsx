    ...
    <div className="space-y-6">
      <div className="card p-5 rounded-2xl border border-[var(--border)] shadow-sm">
        ...
        <p className="text-xs text-[var(--muted)] mt-1.5 leading-relaxed max-w-3xl">
          Live from the backend: password policies, login attempts, session timeouts and audit logging.
        </p>
      </div>
      ...
      <div className="card p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h4 className="text-sm font-extrabold text-[var(--text)]">Security Policies</h4>
        ...
        {loading || !data ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">Password Minimum Length</label>
              {num(data.passwordMinLength, (n) => patch({ passwordMinLength: n }))}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">Password Expiry (days)</label>
              {num(data.passwordExpiryDays, (n) => patch({ passwordExpiryDays: n }))}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">Max Login Attempts</label>
              {num(data.maxLoginAttempts, (n) => patch({ maxLoginAttempts: n }))}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">Session Timeout (minutes)</label>
              {num(data.sessionTimeout, (n) => patch({ sessionTimeout: n }))}
            </div>

            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">Two-Factor Authentication</span>
                <Toggle checked={data.twoFactorEnabled} onChange={(v) => patch({ twoFactorEnabled: v })} />
              </div>
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">IP Restriction</span>
                <Toggle checked={data.ipRestrictionEnabled} onChange={(v) => patch({ ipRestrictionEnabled: v })} />
              </div>
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">Device Restriction</span>
                <Toggle checked={data.deviceRestrictionEnabled} onChange={(v) => patch({ deviceRestrictionEnabled: v })} />
              </div>
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">Audit Logging</span>
                <Toggle checked={data.auditLogEnabled} onChange={(v) => patch({ auditLogEnabled: v })} />
              </div>
            </div>
          </div>
        )}
