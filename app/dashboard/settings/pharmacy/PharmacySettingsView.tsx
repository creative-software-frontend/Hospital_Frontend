// app/dashboard/settings/pharmacy/PharmacySettingsView.tsx
// Live "Pharmacy Settings" page backed by GET/PATCH /api/settings/pharmacy.
...
export function PharmacySettingsView() {
  const [data, setData] = useState<PharmacySetting | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const result = await settingsApi.pharmacy.get();
      setData(result.pharmacySetting);
      setDirty(false);
      setError("");
    } catch (err) {
  ...
  const patch = (partial: Partial<PharmacySetting>) => {
  ...
  const save = async () => {
    ...
    setError("");
    try {
      await settingsApi.pharmacy.update({
      ...
      setDirty(false);
      notify("success", "Pharmacy settings saved.");
      setReloadKey((k) => k + 1);
    } catch (err) {
      setError(errorMessage(err));
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="card p-5 rounded-2xl border border-[var(--border)] shadow-sm">
        ...
        <h3 className="font-black text-xl text-[var(--primary-dark)] mt-0.5">Pharmacy Settings</h3>
        <p className="text-xs text-[var(--muted)] mt-1.5 leading-relaxed max-w-3xl">
          Live from the backend: pharmacy tax, discount limits, batch tracking and stock expiry alerts.
        </p>
      </div>
      ...
      <div className="card p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h4 className="text-sm font-extrabold text-[var(--text)]">Pharmacy &amp; Dispensing</h4>
        ...
        {loading || !data ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              ...
              {money(data.taxPercent, (v) => patch({ taxPercent: v }))}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">Default Discount (%)</label>
              {money(data.defaultDiscount, (v) => patch({ defaultDiscount: v }))}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">Expiry Alert (days)</label>
              {num(data.expiryAlertDays, (n) => patch({ expiryAlertDays: n }))}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">Low Stock Alert</span>
                <Toggle checked={data.lowStockAlert} onChange={(v) => patch({ lowStockAlert: v })} />
              </div>
            </div>
            {toggleRow("Barcode Scanning", data.barcodeEnabled, (v) => patch({ barcodeEnabled: v }))}
            {toggleRow("Batch Tracking", data.batchEnabled, (v) => patch({ batchEnabled: v }))}
          </div>
        )}
      </div>
