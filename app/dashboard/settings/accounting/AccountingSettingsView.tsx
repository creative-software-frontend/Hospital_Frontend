// app/dashboard/settings/accounting/AccountingSettingsView.tsx
// Live "Accounting Settings" page backed by GET/PATCH /api/settings/accounting.
...
export function AccountingSettingsView() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setData(result.accountingSetting);
      setDirty(false);
      setError("");
    } catch (err) {
  ...
  const patch = (partial: Partial<AccountingSetting>) => {
  ...
  const save = async () => {
    ...
    try {
      await settingsApi.accounting.update({
        ...
        voucherEnabled: data.voucherEnabled,
        status: data.status,
      });
      setDirty(false);
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h4 className="text-sm font-extrabold text-[var(--text)]">Chart of Accounts</h4>
        ...
        {loading || !data ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5 sm:col-span-2">
              <label className="block text-xs font-bold text-[var(--muted)]">Fiscal Year</label>
              <input
                type="text"
                maxLength={64}
                value={data.fiscalYear}
                placeholder="e.g. July 2025 - June 2026"
            ...
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              ...
              <input
                ...
                placeholder="e.g. Hospital Standard"
                onChange={(e) => patch({ chartOfAccounts: e.target.value })}
                className={INPUT_CLS}
              />
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 space-y-1.5">
              <label className="block text-xs font-bold text-[var(--muted)]">
                Trial Balance Frequency
              </label>
              {select(
                FREQUENCY_OPTIONS,
                data.trialBalanceFrequency,
                (v) => patch({ trialBalanceFrequency: v as AccountingSetting["trialBalanceFrequency"] }),
              )}
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">Auto-Post to Ledger</span>
                <Toggle checked={data.autoPostToLedger} onChange={(v) => patch({ autoPostToLedger: v })} />
              </div>
            </div>
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--muted)]">Receipt / Payment Voucher</span>
                <Toggle checked={data.voucherEnabled} onChange={(v) => patch({ voucherEnabled: v })} />
              </div>
            </div>
          </div>
        )}
